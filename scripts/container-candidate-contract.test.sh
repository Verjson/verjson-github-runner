...
grep -qx '# Contract: e7140150d29774cbf77be8bc1a764b4c393842f7' "$caller" || fail "caller contract pin differs"
grep -qx '# Contract: e7140150d29774cbf77be8bc1a764b4c393842f7' "$validator" || fail "validator contract pin differs"
[ "$(grep -c 'uses: Verjson/.github/.github/workflows/container-candidate.yml@e7140150d29774cbf77be8bc1a764b4c393842f7' "$caller")" -eq 1 ] || fail "validation does not use the pinned read-only reusable workflow"
[ "$(grep -c 'uses: Verjson/.github/.github/workflows/container-candidate-publish.yml@e7140150d29774cbf77be8bc1a764b4c393842f7' "$caller")" -eq 1 ] || fail "publication does not use the pinned publication reusable workflow"
[ "$(grep -c 'contract-ref: e7140150d29774cbf77be8bc1a764b4c393842f7' "$caller")" -eq 2 ] || fail "caller does not pass the shared pin to both event paths"
...
[ "$(sha256sum "$caller" | cut -d' ' -f1)" = "263118653bb33b76a77c7f05a8d5717c2f63c5f2e91d1dc20001748a204a0542" ] || fail "generated caller was edited"
[ "$(sha256sum "$validator" | cut -d' ' -f1)" = "c702f044f515eaeaab7fdd31116c5644c48ad8030b59bb51cb83c5abc83c24c4" ] || fail "generated validator was edited"

...
# Contract: 19463808ba7cbc4cd80355cf7036ae2d560c7319
set -euo pipefail
...
grep -qx '# Contract: 19463808ba7cbc4cd80355cf7036ae2d560c7319' "$caller" || fail "caller contract pin differs"
grep -qx '# Contract: 19463808ba7cbc4cd80355cf7036ae2d560c7319' "$validator" || fail "validator contract pin differs"
[ "$(grep -c 'uses: Verjson/.github/.github/workflows/container-candidate.yml@19463808ba7cbc4cd80355cf7036ae2d560c7319' "$caller")" -eq 1 ] || fail "validation does not use the pinned read-only reusable workflow"
[ "$(grep -c 'uses: Verjson/.github/.github/workflows/container-candidate-publish.yml@19463808ba7cbc4cd80355cf7036ae2d560c7319' "$caller")" -eq 1 ] || fail "publication does not use the pinned publication reusable workflow"
[ "$(grep -c 'contract-ref: 19463808ba7cbc4cd80355cf7036ae2d560c7319' "$caller")" -eq 2 ] || fail "caller does not pass the shared pin to both event paths"
...
[ "$(sha256sum "$caller" | cut -d' ' -f1)" = "f01ae390b981aad2070edd845ff1157828aca9b37a3a111bdfeb4c123afb1c5b" ] || fail "generated caller was edited"
[ "$(sha256sum "$validator" | cut -d' ' -f1)" = "79bdeb72b0ad2ba4470d58207b3e9bff519e4bfb6e974d4f03b6b79724cd961d" ] || fail "generated validator was edited"
